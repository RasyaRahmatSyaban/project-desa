...
const BeritaService = {
  ...
  getAllBerita: async () => {
    try {
      console.log("Fetching all berita from:", `${API_URL}/berita`)
  ...
  getBeritaByStatus: async (status) => {
    try {
      console.log("Fetching berita by status:", status)
      const response = await api.get(`/berita/status/${status}`)
      console.log("Berita by status response:", response.data)
      return Array.isArray(response.data) ? response.data : []
    } catch (error) {
  ...
  getPublishedBerita: async () => {
    try {
      console.log("Fetching published berita from:", `${API_URL}/berita/status/Dipublikasi`)
      const response = await api.get("/berita/status/Dipublikasi")
      console.log("Published berita response:", response.data)
      return Array.isArray(response.data) ? response.data : []
    } catch (error) {
  ...
  getPaginatedBerita: async (page = 1, limit = 8, status = null) => {
    try {
      ...
      const sortedBerita = allBerita.sort((a, b) => new Date(b.tanggalTerbit) - new Date(a.tanggalTerbit))
  ...
  getLatestBerita: async (limit = 3) => {
    try {
      ...
      return publishedBerita.sort((a, b) => new Date(b.tanggalTerbit) - new Date(a.tanggalTerbit)).slice(0, limit)
    } catch (error) {
  ...
  getImageUrl: (imagePath) => {
    ...
    // Construct the URL to the image using ngrok
    return `${API_URL}/uploads/berita/${filename}`
  },
}
